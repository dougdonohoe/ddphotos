...
of this project to tackle all the various deployment strategies, but we provide two 
deployment options out of the box: Apache via `rsync`, and S3+CloudFront using `aws s3 sync`. 
Both use the same `bin/deploy-photos.sh` script, with
`--s3` selecting S3 mode. My personal site ([photos.donohoe.info](https://photos.donohoe.info))
runs on S3+CloudFront. See [Deployment](docs/DEPLOY.md) for AWS setup details.
